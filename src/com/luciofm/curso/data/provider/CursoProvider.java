 ...
 */
public final class CursoProvider extends ContentProvider {

    private static final String LOG_TAG = CursoProvider.class.getSimpleName();

    private static final boolean ACTIVATE_ALL_LOGS = false;

    protected static final String DATABASE_NAME = "CursoProvider.db";

    public static final String AUTHORITY = "com.luciofm.curso.provider.CursoProvider";
    ...
    private enum UriType {
        CURSO_PACKAGES(CursoPessoas.TABLE_NAME, CursoPessoas.TABLE_NAME, CursoPessoas.TYPE_ELEM_TYPE),
        CURSO_PACKAGES_ID(CursoPessoas.TABLE_NAME + "/#", CursoPessoas.TABLE_NAME, CursoPessoas.TYPE_DIR_TYPE);
    ...
    private class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            Log.d(LOG_TAG, "Creating CursoProvider database");

            // Create all tables here; each class has its own method
            if (ACTIVATE_ALL_LOGS) {
                Log.d(LOG_TAG, "CursoPackages | createTable start");
            }
            CursoPessoas.createTable(db);
            if (ACTIVATE_ALL_LOGS) {
                Log.d(LOG_TAG, "CursoPackages | createTable end");
            }
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (ACTIVATE_ALL_LOGS) {
                Log.d(LOG_TAG, "CursoPackages | upgradeTable start");
            }
            CursoPessoas.upgradeTable(db, oldVersion, newVersion);
            if (ACTIVATE_ALL_LOGS) {
                Log.d(LOG_TAG, "CursoPackages | upgradeTable end");
            }
        }
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            switch (uriType) {
                case CURSO_PACKAGES:
                    insertStmt = db.compileStatement(CursoPessoas.getBulkInsertString());
                    for (ContentValues value : values) {
                        CursoPessoas.bindValuesInBulkInsert(insertStmt, value);
                        insertStmt.execute();
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, 
        ...
        Cursor c = null;
        Uri notificationUri = CursoContent.CONTENT_URI;
        UriType uriType = matchUri(uri);
        Context context = getContext();
        // Pick the correct database for this operation
        SQLiteDatabase db = getDatabase(context);
